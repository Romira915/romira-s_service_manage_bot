...
async fn start(ctx: &Context, msg: &Message, _args: Args) -> CommandResult {
    Ark::terraria_command_exec(SystemctlCommand::Start, ctx, msg).await
}

#[command]
#[description = "ARKサーバの状態を表示する"]
async fn status(ctx: &Context, msg: &Message, _args: Args) -> CommandResult {
    Ark::terraria_command_exec(SystemctlCommand::Status, ctx, msg).await
}

#[command]
#[description = "ARKサーバを停止する"]
async fn stop(ctx: &Context, msg: &Message, _args: Args) -> CommandResult {
    Ark::terraria_command_exec(SystemctlCommand::Stop, ctx, msg).await
}

#[command]
#[description = "ARKサーバを再起動する"]
async fn restart(ctx: &Context, msg: &Message, _args: Args) -> CommandResult {
    Ark::terraria_command_exec(SystemctlCommand::Restart, ctx, msg).await
}
