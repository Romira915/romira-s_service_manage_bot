...
#[prefixes("fourth")]
#[description = "ARK fourth server管理コマンド"]
pub struct ArkFourth;
...
impl ArkFourth {
    pub(crate) async fn generate_url(ctx: &Context) -> String {
        ...
        format!("https://{}/ark-fourth", address.home_server_ip(),)
    }
    ...
    pub(crate) async fn ark_command_exec(
        ...
        let data_read = ctx.data.read().await;
        let url = ArkFourth::generate_url(ctx).await;
...
async fn start(ctx: &Context, msg: &Message, _args: Args) -> CommandResult {
    ArkFourth::ark_command_exec(SystemctlCommand::Start, ctx, msg).await
}

#[command]
#[description = "ARKサーバの状態を表示する"]
async fn status(ctx: &Context, msg: &Message, _args: Args) -> CommandResult {
    ArkFourth::ark_command_exec(SystemctlCommand::Status, ctx, msg).await
}
...
async fn restart(ctx: &Context, msg: &Message, _args: Args) -> CommandResult {
    ArkFourth::ark_command_exec(SystemctlCommand::Restart, ctx, msg).await
}
