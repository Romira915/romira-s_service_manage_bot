...
#[prefixes("third")]
#[description = "ARK third server管理コマンド"]
pub struct ArkThird;
...
impl ArkThird {
    pub(crate) async fn generate_url(ctx: &Context) -> String {
        ...
        format!("https://{}/ark-third", address.home_server_ip(),)
    }
    ...
    pub(crate) async fn ark_command_exec(
        ...
        let data_read = ctx.data.read().await;
        let url = ArkThird::generate_url(ctx).await;
...
async fn start(ctx: &Context, msg: &Message, _args: Args) -> CommandResult {
    ArkThird::ark_command_exec(SystemctlCommand::Start, ctx, msg).await
}

#[command]
#[description = "ARKサーバの状態を表示する"]
async fn status(ctx: &Context, msg: &Message, _args: Args) -> CommandResult {
    ArkThird::ark_command_exec(SystemctlCommand::Status, ctx, msg).await
}
...
async fn restart(ctx: &Context, msg: &Message, _args: Args) -> CommandResult {
    ArkThird::ark_command_exec(SystemctlCommand::Restart, ctx, msg).await
}
